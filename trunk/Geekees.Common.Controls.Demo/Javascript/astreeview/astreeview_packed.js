...
{if(inputId){if(!document.getElementById(inputId))return;thisNode=this.getIcon(document.getElementById(inputId),_rdc.ASTreeView.Consts.cc13);}else{var evt=e||window.event;thisNode=evt.target||evt.srcElement;if(thisNode.tagName=='A')
thisNode=this.getIcon(thisNode.parentNode,_rdc.ASTreeView.Consts.cc13);}
if(thisNode.style.visibility=='hidden')return;var parentNode=thisNode.parentNode;inputId=parentNode.id;if(thisNode.src.indexOf(this.b6)>=0){thisNode.src=thisNode.src.replace(this.b6,this.b7);var ul=parentNode.getElementsByTagName('UL')[0];ul.style.display='block';parentNode.setAttribute("openState",_rdc.ASTreeView.Consts.c1111)
if(this.c22){if(parentNode.getAttribute("is-virtual-node")=="true"){var obj=parentNode;var virtualParentKey=parentNode.getAttribute("virtual-parent-key");var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _rdc.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("virtualParentKey",virtualParentKey);this.__d62(this.b13[ajaxIndex],this.c16);this.b13[ajaxIndex].requestFile=this.c23;var ao=this.b13[ajaxIndex];var fError=this._ajaxError;var fc20=this.__c20;ao.ajaxTrigger=obj;(function(){ao.onCompletion=function(){fc20(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,obj);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();}}}else{thisNode.src=thisNode.src.replace(this.b7,this.b6);parentNode.getElementsByTagName('UL')[0].style.display='none';parentNode.setAttribute("openState",_rdc.ASTreeView.Consts.c11111)}
...
folderIcon.src=this.b2+this.b3;else if(parentNode.getAttribute("openState")==_rdc.ASTreeView.Consts.c1111)
...
this.b17=false;this.az2=0;this.d36();_rdc.a3.cancelEvent(evt);return false;},d36:function()
{if(this.az2>=0&&this.az2<15){this.az2=this.az2+1;this.timer.setTimeout('d36',20);return;}
if(this.az2==15)
{this.b18.style.display='block';this.b18.appendChild(this.b11);}},a12:function(e)
{if(this.az2<15)return;var evt=e||window.event;var thisObj=evt.target||evt.srcElement;if(document.all)e=event;dragDrop_x=this.c7(e)+5;dragDrop_y=this.c8(e)+5;this.b18.style.left=dragDrop_x+'px';this.b18.style.top=dragDrop_y+'px';if(thisObj.tagName=='A'||thisObj.tagName=='IMG')thisObj=thisObj.parentNode;if(thisObj.getAttribute("noChildren")&&thisObj.getAttribute("noChildren").toLowerCase()=="true")
{if(e)
e.stopPropagation();else
window.event.cancelBubble=true;return;}
this.b12=false;var tmpVar=thisObj.getAttribute('disableSiblings');if(!tmpVar)tmpVar=thisObj.disableSiblings;if(tmpVar=='true')this.b12=true;if(thisObj&&thisObj.id)
{this.b17=thisObj;var img=thisObj.getElementsByTagName('IMG')[1];var tmpObj=this.b20;tmpObj.style.display='block';var eventSourceObj=evt.target||evt.srcElement;if(this.b12&&eventSourceObj.tagName=='IMG')eventSourceObj=eventSourceObj.nextSibling;var tmpImg=tmpObj.getElementsByTagName('IMG')[0];if(eventSourceObj.tagName=='A'||this.b12){tmpImg.src=tmpImg.src.replace('ind1','ind2');this.b21=true;tmpObj.style.left=(this.c6(eventSourceObj)+this.b22)+'px';}else{tmpImg.src=tmpImg.src.replace('ind2','ind1');this.b21=false;tmpObj.style.left=(this.c6(eventSourceObj)+this.b23)+'px';}
tmpObj.style.top=(this.c5(thisObj)+this.b24)+'px';}
return false;},d38:function(e)
{var evt=e||window.event;var currentObj=evt.target||evt.srcElement;if(this.az2<15){this.az2=-1;return;}
var showMessage=false;if(this.b17){var countUp=this.d40(this.b17,'up');var countDown=this.d40(this.b11,'down');var countLevels=countUp/1+countDown/1+(this.b21?1:0);if(countLevels>this.b8){this.b17=false;showMessage=true;}}
...
lastImgIcon.src=lastImgIcon.src=this.b2+this.b5;}
tmpObj.parentNode.removeChild(tmpObj);}}else{if(this.b9){this.b10.insertBefore(this.b11,this.b9);}else{this.b10.appendChild(this.b11);}}
...
{this.b20=document.createElement('DIV');this.b20.style.position='absolute';this.b20.style.display='none';var img=document.createElement('IMG');img.src=this.b2+'dragDrop_ind1.gif';img.id='dragDropIndicatorImage';this.b20.appendChild(img);document.body.appendChild(this.b20);},d40:function(obj,direction,stopAtObject){var countLevels=0;if(direction=='up'){while(obj.parentNode&&obj.parentNode!=stopAtObject){obj=obj.parentNode;if(obj.tagName=='UL')countLevels=countLevels/1+1;}
return countLevels;}
...
curSelected=true;nodeInfo.treeNodeText=curA.innerHTML;nodeInfo.treeNodeValue=liValue;nodeInfo.checkedState=parseInt(curCheckedState);nodeInfo.openState=parseInt(curOpenState);nodeInfo.selected=curSelected;nodeInfo.treeNodeType=treeNodeType;if(li.getAttribute("is-virtual-node"))
...
{if(this.d47(obj))return;var img=this.getIcon(obj,_rdc.ASTreeView.Consts.cc13);img.style.visibility='hidden';if(!this.c21){var folderIcon=this.getIcon(obj,_rdc.ASTreeView.Consts.cc15);if(folderIcon){folderIcon.src=this.b2+this.b5;}}},_deleteItemStep2:function(obj)
...
var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _rdc.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("deleteNodeValues",nValue);this.__d62(this.b13[ajaxIndex],this.b85);this.b13[ajaxIndex].requestFile=this.c12;var ao=this.b13[ajaxIndex];var fError=this._ajaxError;var fDeleteComplete=this.__d50;ao.ajaxTrigger=obj;(function(){ao.onCompletion=function(){fDeleteComplete(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,obj);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();}
else
...
return;this._addItemStep2(newNodeText,curNodeValue,curNode);},_addItemStep2:function(newNodeText,parentNodeValue,obj){var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _rdc.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("addNodeText",newNodeText);this.b13[ajaxIndex].setVar("parentNodeValue",parentNodeValue);this.__d62(this.b13[ajaxIndex],this.c15);this.b13[ajaxIndex].requestFile=this.c10;var ao=this.b13[ajaxIndex];var fError=this._ajaxError;var fAddComplete=this.__c19;ao.ajaxTrigger=obj;(function(){ao.onCompletion=function(){fAddComplete(ao);};})();(function(){ao.onError=function(){fError(ajaxIndex,obj);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();},_c19:function(ajaxObject)
{this._hideAjaxIndicatorContainer();var resp=ajaxObject.response;var liStr=resp.match(/<li[.\s\S(?!<li)]*?<\/li>/i);if(!liStr){alert('An error occured while deleting node: '+ajaxObject.response);}else{if(ajaxObject.ajaxTrigger){var liParent=ajaxObject.ajaxTrigger;var imgPM=this.getIcon(liParent,_rdc.ASTreeView.Consts.cc13);if(imgPM){imgPM.style.visibility='visible';imgPM.src=imgPM.src.replace(this.b6,this.b7);}
if(!this.c21){var folderIcon=this.getIcon(liParent,_rdc.ASTreeView.Consts.cc15);if(folderIcon){folderIcon.src=this.b2+this.b4;}}
...
if(this.b41){this.d69(ulParent);}
for(var i=0;i<ulParent.childNodes.length;i++){if(ulParent.childNodes[i].tagName=="LI"){this.d70(ulParent.childNodes[i]);}}
if(ulParent.parentNode&&ulParent.parentNode.tagName=="LI")
this.d70(ulParent.parentNode);if(this.b31){var state=liParent.getAttribute("checkedState");if(state==_rdc.ASTreeView.Consts.c1){for(var i=0;i<ulParent.childNodes.length;i++){if(ulParent.childNodes[i].tagName=="LI")
...
inputObj.style.display='none';inputObj.nextSibling.style.display='';if(inputObj.value.length>0){inputObj.nextSibling.innerHTML=inputObj.value;if(this.b466){if(this.b304!=this.b301){return;}
this.b304=this.b303;var ajaxIndex=this.b13.length;this.b13[ajaxIndex]=new _rdc.AjaxHelper.ajax();this.b13[ajaxIndex].method="GET";this.b13[ajaxIndex].setVar("nodeValue",inputObj.parentNode.getAttribute("treeNodeValue"));this.b13[ajaxIndex].setVar("newNodeText",inputObj.value);this.__d62(this.b13[ajaxIndex],this.b84);this.b13[ajaxIndex].requestFile=this.c11;this.b13[ajaxIndex].onError=function(){this._ajaxError(ajaxIndex);};var ao=this.b13[ajaxIndex];var fEditComplete=this.__d51;(function(){ao.onCompletion=function(){fEditComplete(ao);};})();var fError=this._ajaxError;(function(){ao.onError=function(){fError(ajaxIndex);};})();this._showAjaxIndicatorContainer();this.b13[ajaxIndex].runAJAX();}
else{if(this.b48)
...
{if(document.all)e=event;var elem=e.srcElement||e.target;if(e.keyCode==13){this._d52(false,elem);_rdc.a3.cancelEvent(e);}
if(e.keyCode==27){this._a23(false,elem);}},_d55:function(obj)
{var textBox=document.createElement('INPUT');textBox.className='astreeview-edit-box';textBox.value=obj.innerHTML;obj.parentNode.insertBefore(textBox,obj);textBox.id='textBox'+obj.parentNode.getAttribute("treeNodeValue");_rdc.a3.addEvent(textBox,"blur",this.__d52);_rdc.a3.addEvent(textBox,"keydown",this.__a22);this._d56(obj);},_d56:function(obj)
{this.b304=this.b301;obj.style.display='none';obj.previousSibling.value=obj.innerHTML;obj.previousSibling.style.display='inline';obj.previousSibling.focus();obj.previousSibling.select();},editItem:function(obj1,obj2)
...
{pNode.setAttribute("checkedState",_rdc.ASTreeView.Consts.c11);curCheckbox.src=this.b2+this.b16;}
else{if(checkedNodesCount==0){pNode.setAttribute("checkedState",_rdc.ASTreeView.Consts.c111);curCheckbox.src=this.b2+this.b14;}
else if(checkedNodesCount==totalChildrenCount){pNode.setAttribute("checkedState",_rdc.ASTreeView.Consts.c1);curCheckbox.src=this.b2+this.b15;}
else{pNode.setAttribute("checkedState",_rdc.ASTreeView.Consts.c11);curCheckbox.src=this.b2+this.b16;}}
...
{node.setAttribute("checkedState",_rdc.ASTreeView.Consts.c11);curCheckbox.src=this.b2+this.b16;}
else{if(checkedNodesCount==0){node.setAttribute("checkedState",_rdc.ASTreeView.Consts.c111);curCheckbox.src=this.b2+this.b14;}
else if(checkedNodesCount==totalChildrenCount){node.setAttribute("checkedState",_rdc.ASTreeView.Consts.c1);curCheckbox.src=this.b2+this.b15;}
else{node.setAttribute("checkedState",_rdc.ASTreeView.Consts.c11);curCheckbox.src=this.b2+this.b16;}}}},clearTreeLines:function(obj){obj.className=obj.className.replace(this.b44,"");obj.className=obj.className.replace(this.c17,"");obj.className=obj.className.replace(this.c18,"");obj.className=obj.className.replace(this.b43,"");obj.className=obj.className.replace(this.b42,"");obj.className=obj.className.replace(this.b45,"");},isRootNode:function(node){return node.parentNode==document.getElementById(this.b1);},d70:function(node){var parentUL=_rdc.a3.parent(node);if(!parentUL)
return;if(_rdc.a3.last(parentUL)==node){this.clearTreeLines(node);node.className+=(" "+this.b44);var childUL=_rdc.a3.last(node);if(childUL&&childUL.tagName=="UL")
this.d69(childUL);}
else{this.clearTreeLines(node);node.className+=(" "+this.b43);}
if(this.isRootNode(node)){this.clearTreeLines(node);var siblingNode=_rdc.a3.next(node);if(siblingNode&&siblingNode.tagName=="LI")
node.className+=(" "+this.c18);else
node.className+=(" "+this.c17);}},d69:function(parentUL){if(parentUL.parentNode&&parentUL.parentNode.parentNode){var grandpa=parentUL.parentNode.parentNode
if(_rdc.a3.last(grandpa)==parentUL.parentNode){this.clearTreeLines(parentUL);this.clearTreeLines(parentUL.parentNode);parentUL.className+=(" "+this.b45);parentUL.parentNode.className+=(" "+this.b44);return;}}
this.clearTreeLines(parentUL);parentUL.className+=(" "+this.b42);},getSelectedNodesValue:function(){var result='';var traFunc=function(li){var state=li.getAttribute("selected");if(state=="true"){var val=li.getAttribute("treeNodeValue");result=val;}}
